...
function ToastCard({
  ...
  const enterY = restY + (isTop ? -24 : 24)
  const exitY = restY + (isTop ? -16 : 16)
  ...
  return (
    <motion.div
      ref={cardRef}
      initial={{ opacity: 0, y: enterY, scale: scale * 0.95 }}
      animate={{ opacity: hidden ? 0 : 1, y: restY, scale, zIndex }}
      exit={{ opacity: 0, y: exitY, scale: scale * 0.9 }}
